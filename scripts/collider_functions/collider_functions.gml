...
#region One-way Platform
	...
	function collide_top_one_way_platform(_entity, _resolved_y)
	{
		// Check if the upward movement passed through the bottom of the one-way platform
		if (_entity.yprevious + _entity.bbox_top - _entity.y >= bbox_bottom)
			return max(_resolved_y, bbox_bottom + _entity.y - _entity.bbox_top);
		
		return _resolved_y;
	}
	
	function collide_left_one_way_platform(_entity, _resolved_x)
	{
		// Check if the upward movement passed through the bottom of the one-way platform
		if (_entity.xprevious + _entity.bbox_left - _entity.x >= bbox_right)
			return min(_resolved_x, bbox_right + _entity.x - _entity.bbox_left);
		
		return _resolved_x;
	}
	
	function collide_right_one_way_platform(_entity, _resolved_x)
	{
		// Check if the upward movement passed through the bottom of the one-way platform
		if (_entity.xprevious + _entity.bbox_right - _entity.x >= bbox_left)
			return max(_resolved_x, bbox_left + _entity.x - _entity.bbox_right);
